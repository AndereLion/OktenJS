...
let i = 0;
let recursion = (domElement) => {

    let pForRecursion = document.getElementsByClassName('pForRecursion')[0];
    let buttonLeft = document.createElement('button');
    buttonLeft.innerText = 'Left';
    let buttonRight = document.createElement('button');
    buttonRight.innerText = 'Right';
    let divForRecursionButtons = document.body.getElementsByClassName('divForRecursionButtons')[0];
    divForRecursionButtons.append(buttonLeft, buttonRight);
    ...
    buttonRight.addEventListener('click', () => {
        ...
        buttonLeft.remove();
        let dom = domElement;
        if(dom.children=domElement.children){

        }
        if (domElement.children[i]) {
            console.log(domElement.children[i]);
            recursion(domElement.children[i]);
        } else {

            recursion(domElement.parentElement);
        }
